                    ...
                    <div class="flex flex-wrap items-center gap-y-4">
                        ...
                        <div class="grid w-full gap-4 sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end">
                            ...
                            @if ($order->orderStatus->slug === 'completed' && $order->evaluations->isEmpty())
                                <button class="button-red w-full" data-modal-target="reviewOrder-modal-{{ $order->id }}" data-modal-toggle="reviewOrder-modal-{{ $order->id }}" type="button">Đánh
                                    giá</button>
                            @endif
                                ...
                                <div class="rounded-lg bg-white p-5 shadow-md">
                                    ...
                                    <form action="{{ route('client.order.cancel', $order) }}" method="POST">
                                        ...
                                        <div class="mt-4 flex justify-between">
                                            <button button class="button-gray" data-modal-hide="cancelOrder-modal-{{ $order->id }}" type="button">Không Phải Bây Giờ</button>
                                                    ...
                                                    <div class="form-rating-{{ $group['product']->id }}">
                                                        ...
                                                        @error('ratings.' . $group['product']->id)
                                                            <p class="mt-3 text-center text-sm text-[#D30A0A]">
                                                                {{ $message }}</p>
                                                        @enderror
                                                        <div class="my-3 flex items-center justify-center" id="rating-star-{{ $group['product']->id }}"F>
                                                            <div class="rating-group inline-flex hover:text-[#D30A0A]">
                                                                @for ($i = 1; $i <= 5; $i++)
                                                                    <label aria-label="{{ $i }} star" class="rating__label cursor-pointer p-1" for="rating_{{ $group['product']->id }}_{{ $i }}">
                                                                        <i class="rating__icon--star pointer-events-none text-[#D30A0A] group-hover:text-[#D30A0A]">
                                                                            @svg('tabler-star-filled', 'icon-lg')
                                                                        </i>
                                                                    </label>
                                                                    <input @if ($i === 5) checked @endif class="rating__input" id="rating_{{ $group['product']->id }}_{{ $i }}" name="ratings[{{ $group['product']->id }}]" type="radio" value="{{ $i }}" />
                                                                @endfor
                                                            </div>
                                                        </div>
                                                        {{-- Comment --}}
                                                        <div class="mb-4">
                                                            <textarea class="text-area resize-none text-sm" name="comments[{{ $group['product']->id }}]" placeholder="Viết đánh giá..." rows="6"></textarea>
                                ...
                                <div class="text-right">
                                    <div>
                                        ...
                                        <div class="mb-4 flex items-center justify-between gap-32 text-sm">
                                            ...
                                            <p class="text-end font-medium">{{ number_format($order->shipping_fee) }}₫</p>
                                        </div>
                                        <div class="mb-4 flex items-center justify-between gap-32 text-sm">

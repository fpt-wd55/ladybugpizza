        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500">
               ...
               <tr class="border-b hover:bg-gray-100">
                   <td class="flex items-center lg:px-4 lg:py-2 text-gray-900  mt-3">
                       <a href="{{route('admin.memberships.edit',$membership)}}">
                           <img loading="lazy" src="{{ asset('storage/uploads/avatars/'. $membership->user->avatar) }}" alt="Avatar"
                               class="img-circle img-sm lg:w-11 lg:h-11 lg:mr-3 rounded border-2 hover:border-[#D30A0A] " {{ Auth::user()->avatar() }}>
                       </a>
                       <a href="{{route('admin.memberships.edit',$membership)}}" class="hover:text-[#D30A0A]">
                           <div class="grid grid-flow-row">
                               <span class="text-xs ml-1 lg:ml-0 lg:text-sm">{{$membership->user->username}}</span>
                               <span class=" text-gray-500 hidden md:block">{{$membership->user->email}}</span>
                           </div>
                       </a>
                   </td>
                   <td class=" text-gray-900 whitespace-nowrap text-xs lg:text-sm ">{{$membership->user->fullname}}</td>
                   <td class="lg:px-4 lg:py-2 text-gray-900 whitespace-nowrap ">  
                    <div class="flex flex-col items-center">        
                        <img loading="lazy" src="{{ asset('storage/uploads/ranks/'.$membership->rank_img) }}" alt="" class=" img-circle w-6 h-6 md:w-14 md:h-14">
                   ...
                   <td class="px-0 py-0 lg:px-4 lg:py-2 text-gray-900 whitespace-nowrap ">
                   ...
                   <td class="lg:px-4 lg:py-3 items-center justify-end ">
                       ...
                       <div id="{{$membership->id}}-dropdown"
                           ...
                           <ul class="py-1 text-sm text-gray-700" aria-labelledby="{{$membership->id}}">
                               <li>
                                   <a href="{{route('admin.memberships.edit',$membership)}}"
                                       class="block py-1 px-2 md:py-2 md:px-4 hover:bg-gray-100">Chi tiết</a>
                               </li>

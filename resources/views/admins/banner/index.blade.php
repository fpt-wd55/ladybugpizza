...
@section('content')
    ...
    <div class="relative mt-5 overflow-hidden bg-white shadow sm:rounded-lg">
        ...
        <div
            ...
            <div
                class="flex flex-shrink-0 flex-col space-y-3 md:flex-row md:items-center md:space-x-3 md:space-y-0 lg:justify-end">
                <form action="{{ route('admin.banner.search') }}" class="flex w-full md:w-40 lg:w-64">
                ...
                <div class="flex w-full items-center md:w-auto">
                    <button class="flex w-full items-center justify-center button-light" data-modal-target="filterDropdown"
                        data-modal-toggle="filterDropdown" type="button">
                        @svg('tabler-filter-filled', 'w-5 h-5 me-2')
                        Bộ lọc
                    </button>
                    <form action="{{ route('admin.banner.filter') }}" aria-hidden="true"
                            ...
                            <div class="relative rounded-lg bg-white shadow">
                                ...
                                <div class="px-4 md:px-6">
                                    ...
                                    <ul class="space-y-2 text-sm">
                                        <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                            <li class="flex items-center">
                                                <input @if (in_array(1, request()->input('filter_is_local_page', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    id="local" name="filter_is_local_page[]" type="checkbox"
                                                    value="1">
                                                <label class="ml-2 text-sm font-medium text-gray-900" for="local">Trang
                                                    cục bộ</label>
                                            </li>
                                            <li class="flex items-center">
                                                <input @if (in_array(2, request()->input('filter_is_local_page', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    id="external" name="filter_is_local_page[]" type="checkbox"
                                                    value="2">
                                    ...
                                    <ul class="space-y-2 text-sm">
                                        <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                            <li class="flex items-center">
                                                <input @if (in_array(1, request()->input('filter_status', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    id="active" name="filter_status[]" type="checkbox" value="1">
                                                <label class="ml-2 text-sm font-medium text-gray-900" for="active">Hoạt
                                                    động</label>
                                            </li>
                                            <li class="flex items-center">
                                                <input @if (in_array(2, request()->input('filter_status', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    id="inactive" name="filter_status[]" type="checkbox" value="2">
                                                <label class="ml-2 text-sm font-medium text-gray-900"
                                                    for="inactive">Khóa</label>
                                            </li>
                                        </div>
                                    </ul>
                                </div>

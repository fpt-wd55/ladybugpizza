    ...
    <div class="mt-5 bg-white relative shadow sm:rounded-lg overflow-hidden">
        <div class="overflow-x-auto ">
            <div
                ...
                @endif
                {{-- <a href="{{ route('admin.invoices.create') }}" class="button-blue">
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3">Giao dịch</th>
                ...
                <tbody>
                    @forelse ($invoices as $invoice)
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="px-4 py-2 text-gray-900 whitespace-nowrap">{{$invoice->transaction}}</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="{{ $invoice->id }}" data-dropdown-toggle="{{ $invoice->id }}-dropdown"
                                ...
                                <div id="{{ $invoice->id }}-dropdown"
                                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow">
                                    <ul class="py-1 text-sm text-gray-700" aria-labelledby="{{ $invoice->id }}">
                                        <li>
                                    ...
                                    <div class="py-1">
                                        <a href="#" data-modal-target="delete-modal-{{ $invoice->id }}"
                                            data-modal-toggle="delete-modal-{{ $invoice->id }}"
                                            class="cursor-pointer block py-2 px-4 text-sm text-red-500 hover:bg-gray-100">Xem đơn hàng</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
                        {{-- start modal --}}
                        <div id="delete-modal-{{ $invoice->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                        data-modal-hide="delete-modal-{{ $invoice->id }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <button data-modal-hide="delete-modal-{{ $invoice->id }}" type="button"

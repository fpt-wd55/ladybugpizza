    ...
    <div class="mt-5 bg-white shadow sm:rounded-lg">
        <div
            ...
            <div
                ...
                <form action="{{ route('admin.order.search') }}" class="flex w-full md:w-40 lg:w-64">
                ...
                <div class="flex items-center justify-end gap-3">
                    <button class="button-light flex w-full items-center justify-center" data-modal-target="filterDropdown"
                        data-modal-toggle="filterDropdown" type="button">
                        @svg('tabler-filter-filled', 'w-5 h-5 me-2')
                        Bộ lọc
                    </button>
                    <a class="button-light flex w-full items-center justify-center min-w-40"
                        href="{{ route('admin.orders.export') }}">
                        @svg('tabler-file-export', 'w-5 h-5 me-2')
                        Xuất dữ liệu
                    </a>
                </div>
                <form action="{{ route('admin.orders.filter') }}" aria-hidden="true"
                            ...
                            <div class="px-4 md:px-6">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                        @foreach ($orderStatuses as $status)
                                            <li class="flex items-center">
                                                <input @if (in_array($status->id, request()->input('filter_status', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    name="filter_status[]" type="checkbox" value="{{ $status->id }}">
                                                <label class="ml-2 text-sm font-medium text-gray-900"
                                                    for="active">{{ $status->name }}</label>
                                            </li>
                                        @endforeach
                                    </div>
                                </ul>
                                ...
                                <ul class="space-y-2 text-sm">
                                    <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                        @foreach ($paymentMethods as $paymentMethod)
                                            <li class="flex items-center">
                                                <input @if (in_array($paymentMethod->id, request()->input('filter_paymentMethod', []))) checked @endif
                                                    class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                    name="filter_paymentMethod[]" type="checkbox"
                                                    value="{{ $paymentMethod->id }}">
                                                <label class="ml-2 text-sm font-medium text-gray-900"
                                                    for="active">{{ $paymentMethod->name }}</label>
                                            </li>
                                        @endforeach
                                    </div>
                                </ul>
                                ...
                                <div class="flex items-center">
                                    <div>
                                        <input
                                            ...
                                            name="filter_amount_min" placeholder="1.000 đ" type="number"
                                    ...
                                    <div>
                                        <input
                                            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 ps-3 text-sm text-gray-900 focus:ring-0"
                                            name="filter_amount_max" placeholder="100.000.000 đ" type="number"
                                ...
                                <div class="flex items-center">
                                    <div>
                                        <input
                                            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 ps-3 text-sm text-gray-900 focus:ring-0"
                                            name="filter_date_min" placeholder="mm/dd/yyyy" type="date"
                                    ...
                                    <div>
                                        <input
                                            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 ps-3 text-sm text-gray-900 focus:ring-0"
                                            name="filter_date_max" placeholder="mm/dd/yyyy" type="date"
        ...
        <div class="no-scrollbar mb-4 overflow-x-auto border-gray-200 text-left">
            ...
            <ul class="flex">
                <li class="relative mx-4 me-6 min-w-fit">
                    <a aria-current="page"
                        class="{{ request()->routeIs('admin.orders.index') && request('tab') === null ? 'border-[#D30A0A] text-[#D30A0A]' : 'border-transparent' }} inline-block rounded-t-lg border-b-2 pb-1"
                ...
                @foreach ($orderStatuses as $status)
                    <li class="relative mx-4 me-6 min-w-fit">
                        <a aria-current="page"
                            class="{{ request()->get('tab') === $status->slug ? 'border-[#D30A0A] text-[#D30A0A]' : 'border-transparent' }} inline-block rounded-t-lg border-b-2 pb-1"
                    ...
                    @forelse ($orders as $order)
                        <tr class="border-b hover:bg-gray-100">
                            <td class="md:px-4 md:py-2">
                                <a class="font-medium text-gray-800 hover:text-red-600"
                                    data-modal-target="order-modal-{{ $order->id }}"
                                    data-modal-toggle="order-modal-{{ $order->id }}"
                                    href="#">#{{ $order->code }}</a>
                            </td>
                            <td class="md:px-4 md:py-2">
                                <a class="mb-2 font-medium text-gray-800 hover:text-red-600"
                                    href="{{ route('admin.users.show', $order->user->id) }}">{{ $order->fullname }}</a>
                            ...
                            <td class="flex items-center justify-end px-4 py-3">
                                <button
                                    ...
                                    data-dropdown-toggle="{{ $order->id }}-dropdown" id="{{ $order->id }}"
                                    type="button">
                                    ...
                                    <ul aria-labelledby="{{ $order->id }}" class="py-1 text-sm text-gray-700">
                                        <li>
                                            <a class="block px-4 py-2 hover:bg-gray-100"
                                                data-modal-target="order-modal-{{ $order->id }}"
                                                data-modal-toggle="order-modal-{{ $order->id }}" href="#">Chi
                                                tiết </a>
                                        </li>
                                        <li>
                                            <a class="block px-4 py-2 hover:bg-gray-100"
                                                href="{{ route('admin.orders.edit', $order->id) }}">Cập nhật
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                        ...
                        <div aria-hidden="true"
                            ...
                            id="order-modal-{{ $order->id }}" tabindex="-1">
                            <div class="relative h-auto w-full max-w-5xl p-4">
                                <div
                                    class="no-scrollbar relative h-[480px] overflow-y-auto rounded-lg bg-white p-5 shadow md:p-8">
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between">
                                            <h1 class="text-xl font-semibold">Đơn hàng #{{ $order->code }}</h1>
                                            <button
                                                class="ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900"
                                                data-modal-hide="order-modal-{{ $order->id }}" type="button">
                                        ...
                                        <div class="rounded-lg pl-4">
                                            ...
                                            <p
                                                class="{{ $colorClass }} mx-2 mt-1.5 inline-block shrink-0 items-center rounded px-2.5 py-0.5 text-xs font-medium text-white">
                                                {{ $order->orderStatus->name }}
                                            </p>
                                        </div>
                                        {{-- user_id --}}
                                                            ...
                                                            <div class="w-full">
                                                                <div class="grid grid-flow-row">
                                                                    <p class="flex justify-between">
                                                                        ...
                                                                        <span
                                                                            class="font-medium">{{ number_format($orderItem->quantity * $orderItem->price) }}₫</span>
                                                                    </p>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 mt-3">
                                                ...
                                                <div class="mt-5 md:mt-0">
                                                    ...
                                                    <div class="mb-2 flex items-center justify-between gap-32 text-sm">
                                                        ...
                                                        <p class="font-medium">{{ number_format($order->shipping_fee) }}
                                                            ₫</p>
                                                    </div>
                                                    <div class="mb-2 flex items-center justify-between gap-32 text-sm">
                                                        <p class="">Giảm giá</p>

        ...
        <div class="overflow-x-auto">
            <div
                ...
                <div
                    ...
                    <a class="button-light" href="{{ route('admin.promotions.export') }}">
            ...
            <div
                    ...
                    <div class="flex w-full items-center space-x-3 md:w-full">
                        <form action="{{ route('admin.promotions.bulkAction') }}" method="POST">
                            ...
                            <div class="hidden" id="actionButtons">
                                ...
                                <h2 class="flex items-center text-base font-medium italic text-gray-700"
                                    id="selectedItems">
                                </h2>
                            </div>
                        </form>
                ...
                <div
                    ...
                    <div class="flex w-full items-center space-x-3 md:w-auto">
                        <button class="button-light flex w-full items-center justify-center"
                                data-modal-target="filterDropdown" data-modal-toggle="filterDropdown" type="button">
                            @svg('tabler-filter-filled', 'w-5 h-5 me-2')
                            Bộ lọc
                        </button>
                        <form action="{{ route('admin.promotions.filter') }}" aria-hidden="true"
                              class="fixed inset-0 z-50 hidden h-modal w-full overflow-y-auto overflow-x-hidden p-4 md:h-full"
                              id="filterDropdown" method="get" tabindex="-1">
                                ...
                                <div class="relative rounded-lg bg-white shadow">
                                    <div class="flex items-start justify-between rounded-t px-6 py-4">
                                        ...
                                        <button
                                            ...
                                            data-modal-toggle="filterDropdown" type="button">
                                    ...
                                    <div class="px-4 md:px-6">
                                        ...
                                        <ul class="space-y-2 text-sm">
                                            <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                                <li class="flex items-center">
                                                    <input
                                                        @if (in_array(1, request()->input('filter_discount_type', []))) checked
                                                        @endif class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                        id="discount-type-1" name="filter_discount_type[]"
                                                        type="checkbox" value="1">
                                                    <label class="ml-2 text-sm font-medium text-gray-900"
                                                           for="discount-type-1">Giảm theo %</label>
                                                </li>
                                                <li class="flex items-center">
                                                    <input
                                                        @if (in_array(2, request()->input('filter_discount_type', []))) checked
                                                        @endif class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                        id="discount-type-2" name="filter_discount_type[]"
                                                        type="checkbox" value="2">
                                                    <label class="ml-2 text-sm font-medium text-gray-900"
                                                           for="discount-type-2">Giảm theo giá
                                                        tiền</label>
                                                </li>
                                            </div>
                                        </ul>
                                        ...
                                        <ul class="space-y-2 text-sm">
                                            <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                                                <li class="flex items-center">
                                                    <input
                                                        @if (in_array(0, request()->input('filter_range', []))) checked
                                                        @endif class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                        id="range-0" name="filter_range[]" type="checkbox" value="0">
                                                    <label class="ml-2 text-sm font-medium text-gray-900" for="range-0">Tất
                                                        cả</label>
                                                </li>
                                                @foreach ($ranks as $rank)
                                                    <li class="flex items-center">
                                                        <input
                                                            @if (in_array($rank->id, request()->input('filter_range', []))) checked
                                                            @endif class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                                            id="range-{{ $rank->id }}" name="filter_range[]"
                                                            type="checkbox" value="{{ $rank->id }}">
                                                        <label class="ml-2 text-sm font-medium text-gray-900"
                                                               for="range-{{ $rank->id }}">Thành viên
                                                            {{ $rank->name }}</label>
                                                    </li>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="flex items-center">
                                            <div>
                                                <input
                                                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 ps-3 text-sm text-gray-900 focus:ring-0"
                                                    name="filter_date_min" placeholder="1.000 đ" type="date"
                                            ...
                                            <div>
                                                <input
                                                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 ps-3 text-sm text-gray-900 focus:ring-0"
                                                    name="filter_date_max" placeholder="100.000.000 đ" type="date"
            ...
            <table class="w-full text-left text-sm text-gray-500">
                ...
                <tr>
                    <th class="p-4" scope="col">
                        <div class="flex items-center">
                            <input class="text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                   id="table-checkbox-all" type="checkbox">
                        </div>
                    </th>
                    <th class="px-4 py-3" scope="col">Mã giảm giá</th>
                    <th class="px-4 py-3" scope="col">Đối tượng áp dụng</th>
                ...
                @forelse ($promotions as $promotion)
                    <tr class="border-b hover:bg-gray-100">
                        <td class="w-4 px-4 py-3">
                            <div class="flex items-center">
                                <input
                                    class="table-item-checkbox text-primary-600 h-4 w-4 rounded border-gray-300 bg-gray-100 focus:ring-0"
                                    id="table-item-checkbox-{{ $promotion->id }}" onclick="event.stopPropagation()"
                                    type="checkbox" value="{{ $promotion->id }}">
                            </div>
                        </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-900">
                            <a class="font-medium hover:text-red-600"
                               data-modal-target="detail-modal-{{ $promotion->id }}"
                               data-modal-toggle="detail-modal-{{ $promotion->id }}" href="#">{{ $promotion->code }}</a>
                            <p>{{ $promotion->name }}</p>
                        </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-900">
                            <p>
                                ...
                                @else
                                    <span
                                        class="badge-default">Thành viên {{ $promotion->rank?->name ?? 'Không xác định' }}</span>
                                @endif
                            </p>
                        </td>
                        ...
                        <td class="flex items-center justify-end px-4 py-3">
                            <button
                                ...
                                data-dropdown-toggle="{{ $promotion->id }}-dropdown" id="{{ $promotion->id }}"
                                type="button">
                                @svg('tabler-dots', 'w-5 h-5')
                            </button>
                            <div class="z-10 hidden w-44 divide-y divide-gray-100 rounded bg-white shadow"
                                 id="{{ $promotion->id }}-dropdown">
                                <ul aria-labelledby="{{ $promotion->id }}" class="py-1 text-sm text-gray-700">
                                    <li>
                                        <a class="block px-4 py-2 hover:bg-gray-100"
                                           data-modal-target="detail-modal-{{ $promotion->id }}"
                                           data-modal-toggle="detail-modal-{{ $promotion->id }}" href="#">Chi tiết </a>
                                    </li>
                                    @if (!$promotion->orders()->exists())
                                        <li>
                                            <a class="block cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                                               href="{{ route('admin.promotions.edit', $promotion->id) }}">
                                                Cập nhật
                                            </a>
                                        </li>
                                        <li>
                                            <a class="block cursor-pointer px-4 py-2 text-sm text-red-500 hover:bg-gray-100"
                                               data-modal-target="delete-modal-{{ $promotion->id }}"
                                               data-modal-toggle="delete-modal-{{ $promotion->id }}" href="#">Xóa</a>
                                        </li>
                                    @endif
                                </ul>
                            </div>
                    ...
                    <div
                        ...
                        id="delete-modal-{{ $promotion->id }}" tabindex="-1">
                        <div class="relative max-h-full w-full max-w-md p-4">
                            <div class="relative rounded-lg bg-white shadow">
                                <button
                                    ...
                                    data-modal-hide="delete-modal-{{ $promotion->id }}" type="button">
                                ...
                                <div class="p-4 text-center md:p-5">
                                    ...
                                    <button
                                        ...
                                        data-modal-hide="delete-modal-{{ $promotion->id }}" type="button">Không
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div aria-hidden="true"
                         class="fixed left-0 right-0 top-10 z-50 hidden h-modal w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0 md:h-full"
                         id="detail-modal-{{ $promotion->id }}" tabindex="-1">
                        <div class="relative max-h-full w-full max-w-md p-4">
                            <div
                                class="no-scrollbar relative h-full overflow-y-auto rounded-lg bg-white p-4 shadow sm:p-5">
                                <div>
                                    <div class="container mx-auto mb-10">
                                        <div
                                            class="px-15 relative rounded-lg bg-gradient-to-br from-[#bf0808] to-[#f52929] py-8 text-center text-white shadow-md">
                                            ...
                                            <div class="flex items-center justify-center space-x-2">
                                                <span class="rounded-l border border-dashed px-4 py-2"
                                                      id="promotion-{{ $promotion->code }}">{{ $promotion->code }}</span>
                                                <button
                                                    class="cursor-pointer rounded-r border border-white bg-white px-2 py-2 text-red-600 transition hover:bg-slate-200"
                                                    ...
                                                    onclick="notiCopied()">Sao chép
                                                </button>
                                            </div>
                                            <div
                                                class="absolute left-0 top-1/2 -ml-6 h-12 w-12 -translate-y-1/2 transform rounded-full bg-white">
                                            </div>
                                            <div
                                                class="absolute right-0 top-1/2 -mr-6 h-12 w-12 -translate-y-1/2 transform rounded-full bg-white">
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div class="rounded-lg">
                                                ...
                                                <p class="mb-6 mt-3 text-gray-800">
                                                        <span
                                                            class="{{ $promotion->status == 1 ? 'bg-green-500' : 'bg-red-500' }} inline-flex shrink-0 items-center rounded px-2.5 py-0.5 text-xs font-medium text-white">
                        ...
                        @empty
                            <td class="py-4 text-center text-base" colspan="6">
                                <div
                                    class="flex h-80 w-full flex-col items-center justify-center rounded-lg bg-white p-6">
